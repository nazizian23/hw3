    ...
    while( getline(inFile, line))
    {
        ...
        if(line == "GATES")
        {
            ...
            for(int i = 0;i<n;i++)
            {
                ...
                if(s_type=="NOT"){
                    std::string s_in;
                    getline(ss,s_in,',');//Get input wire ID
                    std::string s_output;
                    getline(ss, s_output,',');//Get output ID
                    m_gates.push_back(new NotGate(m_wires[stoi(s_in)],m_wires[stoi(s_output)]));//Create the not gate
                }
            }
        }
        if(line == "INJECT")
